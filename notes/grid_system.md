...
3.  **Visual Authority:** Styling rules (colors, rims, highlighting) are centralized in the `GridVisualizationManager`, avoiding scattered visual logic.
4.  **Tool-Based Interaction:** Interaction is handled by dedicated tools (`PathfindingTool`, `GridCreator`) rather than the core manager, keeping classes focused (Single Responsibility Principle).
...
*   **`HexData`**: The atomic unit of the grid.
    ...
    *   **State Management**: Contains a `HashSet<string>` to track active abstract states (e.g., `"Selected"`, `"Hovered"`, `"PathfindingCandidate"`).
...
*   **`GridVisualizationManager`**:
    *   **The Styling Authority**: Defines how abstract states (strings) translate to visual properties (Rim Color, Width, Pulsation).
    *   **Priority System**: Uses a list of `StateSetting` structs (loaded from JSON), each with a priority. When a hex has multiple states (e.g., `"Hovered"` AND `"Selected"`), the manager applies the settings of the highest-priority state.
    *   **Default Fallback**: Automatically falls back to a `"Default"` visual style if no specific states are active.
    *   **JSON Configuration**: Known state visualizations are defined in `Assets/Data/StateVisuals/default_states.json`.
...
*   **`PathfindingTool`**:
    ...
    *   Has methods like `SelectHex`, `HandleHighlighting`.
    *   It **only** modifies the `HexData` states (adding/removing state strings). It does *not* touch materials directly. The View layer reacts to these state changes.
