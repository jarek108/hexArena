...
namespace HexGame.Tests
{
    public class ToolInteractionTests
    {
        ...
        public IEnumerator PathfindingTool_WhenActive_HighlightsHoveredHex()
        {
            ...
            Assert.IsTrue(hexA.Data.States.Contains("Hovered"), "Hex A should be in Hovered state.");
            Assert.IsFalse(hexB.Data.States.Contains("Hovered"), "Hex B should not be in Hovered state.");

            toolManager.ManualUpdate(hexB);
            yield return null;

            Assert.IsFalse(hexA.Data.States.Contains("Hovered"), "Hex A should no longer be in Hovered state.");
            Assert.IsTrue(hexB.Data.States.Contains("Hovered"), "Hex B should now be in Hovered state.");
        }
        ...
        public IEnumerator TerrainTool_WhenActive_HighlightsHoveredHexes()
        {
            ...
            Assert.IsTrue(centerHex.Data.States.Contains("Hovered"), "Center hex should be highlighted.");
            Assert.IsTrue(neighborHex.Data.States.Contains("Hovered"), "Neighbor hex should be highlighted by brush size.");
            Assert.IsFalse(distantHex.Data.States.Contains("Hovered"), "Distant hex should not be highlighted.");
        }
    }
}