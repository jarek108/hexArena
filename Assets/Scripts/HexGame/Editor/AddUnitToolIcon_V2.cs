...
{
    [InitializeOnLoad]
    public static class AddUnitToolIcon_V2
    {
        static AddUnitToolIcon_V2()
        {
            // Delay to ensure scene is loaded
            EditorApplication.delayCall += AddIcon;
        }
        ...
        public static void AddIcon()
        {
            ...
            if (iconManager == null || toolManager == null) return;

            // Check if already exists
            if (iconManager.icons.Exists(i => i.iconName == "Units")) return;
            ...
            Debug.Log("Unit tool icon added to IconManager (V2).");
